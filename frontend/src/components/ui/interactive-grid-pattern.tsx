...
export function InteractiveGridPattern({
    ...
    strokeDasharray = "0",
    squares = [50, 50],
    squaresClassName = "fill-blue-500/50",
    ...props
}: InteractiveGridPatternProps) {
    const [squarePositions, setSquarePositions] = useState<Array<{ id: number; pos: [number, number] }>>([])
            ...
            <svg className="absolute inset-0 h-full w-full">
                ...
                {squarePositions.map(({ id, pos }) => (
                    <rect
                        ...
                        y={pos[1]}
                        strokeWidth="0"
                        className={cn("transition-colors duration-300", squaresClassName)}
                    />
                ))}
            </svg>
        </div>
